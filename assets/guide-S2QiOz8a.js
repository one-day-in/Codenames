...
import{t as d,D as l}from"./i18n-BwlAWuOW.js";import{_ as A,o as I}from"./fullscreen-D1PshyVK.js";import{R as p,k as M,c as R,a as D,b as G}from"./keepAlive-D1x8_-Il.js";import{a as H}from"./fitText-DhGiXhRH.js";const N=8;function O(){const n=new URLSearchParams(window.location.search);return{roomId:n.get("room"),token:n.get("token"),team:n.get("team")}}async function q(n){const{roomId:r,token:g,team:a}=O();if(!r||!g||!a||a!=="resonant"&&a!=="dissonant"){n.innerHTML=`<div class="waiting-screen">
            <p class="waiting-screen__hint">${d(l).scanQr}</p>
        </div>`;return}const{supabase:f}=await A(async()=>{const{supabase:e}=await import("./i18n-BwlAWuOW.js").then(s=>s.a);return{supabase:e}},__vite__mapDeps([0,1])),{data:v,error:L}=await f.from("rooms").select("id, guest_token").eq("id",r).eq("guest_token",g).maybeSingle();if(!v||L){n.innerHTML=`<div class="waiting-screen">
