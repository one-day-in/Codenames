import{o as k,b as q,e as O,r as I,d as W}from"./sanitize-DvpzgQ19.js";import{t as f,D as b,n as j,d as F,I as R}from"./url-BsD_c2-s.js";import{i as x}from"./initGuestPage-DUAjKxWq.js";import"./entry-TySTgPe0.js";const D=8;async function K(a){const h=await x(a,{roleType:"guide",invalidParamsHtml:`<div class="waiting-screen">
            <p>${f(b).scanQr}</p>
        </div>`});if(!h)return;const{presence:p,store:l,team:d}=h;let m=!1,g=new Set;function L(i=b){m=!1,g=new Set,a.innerHTML=`<div class="waiting-screen">
            <p>${f(i).waitingGame}</p>
    ...
    `,a.querySelectorAll(".guide .grid .cell").forEach((e,s)=>{m&&(!T.has(s)||g.has(s)||e.classList.add("cell--reveal-anim"))}),g=T,m=!0,requestAnimationFrame(()=>{a.querySelectorAll(".cell").forEach(e=>W(e))}),a.querySelectorAll(".guide__num-btn:not([disabled])").forEach(e=>{e.addEventListener("click",()=>l.setGuideLimit(parseInt(e.dataset.limit,10)))}),(y=a.querySelector(".turn-transition-overlay"))==null||y.addEventListener("click",()=>{l.dismissTurnTransition()})}function v({state:i,language:o}={}){const t=i??l.getState(),r=o??l.getLanguage();if(!t){L(r);return}w(t,r)}l.subscribe(v),p.onChange(()=>v()),k(()=>v())}export{K as initGuide};
