import{o as k,d as q,e as O,r as I,h as W}from"./sanitize-rh7sClml.js";import{t as f,D as p,n as j,d as F,I as R}from"./url-DF3vUGGd.js";import{i as x}from"./initGuestPage-DvQDcTdk.js";import"./entry-bCZlYvsl.js";const D=8;async function K(a){const h=await x(a,{roleType:"guide",invalidParamsHtml:`<div class="waiting-screen">
            <p>${f(p).scanQr}</p>
        </div>`});if(!h)return;const{presence:L,store:l,team:d}=h;let m=!1,g=new Set;function b(i=p){m=!1,g=new Set,a.innerHTML=`<div class="waiting-screen">
            <p>${f(i).waitingGame}</p>
        </div>`}function w(i,o){var y;const t=f(o),r=i.turn,C=r.team===d,E=r.guideLimit!==null,n=C&&!E&&!i.gameOver&&!i.turnTransition,c=j(i.teamEffects)[d],A=c.allowTwoWordClue,_=c.forcedGuideLimit,S=c.hideEnemyColors,B=c.hideNightmare;document.body.classList.remove("team-resonant","team-dissonant"),document.body.classList.add(`team-${d}`);const G=F(d,o),H=n?`${t.guide}: ${t.chooseLimit}`:`${t.guide}: ${R.eyeClosed}`,$=[n&&_===1?t.guideForcedOneMove:null,n&&A?t.guideTwoWordClue:null,S?t.guideBlurActive:null,B?t.guideNightmareHidden:null].filter(Boolean),N=$.length?`<div class="guide__meta ${n?"guide__meta--active":"guide__meta--muted"}">${$.join(" • ")}</div>`:"",M=Array.from({length:D},(e,s)=>{const u=s+1;return`
            <button
    ...
    `,a.querySelectorAll(".guide .grid .cell").forEach((e,s)=>{m&&(!T.has(s)||g.has(s)||e.classList.add("cell--reveal-anim"))}),g=T,m=!0,requestAnimationFrame(()=>{a.querySelectorAll(".cell").forEach(e=>W(e))}),a.querySelectorAll(".guide__num-btn:not([disabled])").forEach(e=>{e.addEventListener("click",()=>l.setGuideLimit(parseInt(e.dataset.limit,10)))}),(y=a.querySelector(".turn-transition-overlay"))==null||y.addEventListener("click",()=>{l.dismissTurnTransition()})}function v({state:i,language:o}={}){const t=i??l.getState(),r=o??l.getLanguage();if(!t){b(r);return}w(t,r)}l.subscribe(v),L.onChange(()=>v()),k(()=>v())}export{K as initGuide};
